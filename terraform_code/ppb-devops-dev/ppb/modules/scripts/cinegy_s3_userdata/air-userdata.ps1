...
if($AZ -eq "eu-west-1a"){
	$file = Get-S3Object -BucketName $bucket -Key $Cinegy_AEMain_Config_0_S3path
	Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\Cinegy'
	Copy-Item -Path C:\Cinegy\$Cinegy_AEMain_Config_0_S3path C:\ProgramData\Cinegy\CinegyAir\Config -PassThru
	$file = Get-S3Object -BucketName $bucket -Key $Cinegy_AEMain_Config_1_S3path
	Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\Cinegy'	
	Copy-Item -Path C:\Cinegy\$Cinegy_AEMain_Config_1_S3path C:\ProgramData\Cinegy\CinegyAir\Config -PassThru
}elseif($AZ -eq "eu-west-1b"){
	$file = Get-S3Object -BucketName $bucket -Key $Cinegy_AEBackup_Config_0_S3path
	Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\Cinegy'
	Copy-Item -Path C:\Cinegy\$Cinegy_AEBackup_Config_0_S3path C:\ProgramData\Cinegy\CinegyAir\Config -PassThru
	$file = Get-S3Object -BucketName $bucket -Key $Cinegy_AEBackup_Config_1_S3path
	Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\Cinegy'	
	Copy-Item -Path C:\Cinegy\$Cinegy_AEBackup_config_1_s3path C:\ProgramData\Cinegy\CinegyAir\Config -PassThru
}
...
Install-Product -PackageName Cinegy-Air-$Cinegy_Air_Version -VersionTag release
...
$file = Get-S3Object -BucketName $bucket -Key $Cinegy_AudioMatrix_Config_S3path 
Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\Cinegy'	
Copy-Item -Path C:\Cinegy\$Cinegy_AudioMatrix_Config_S3path "C:\Program Files\Cinegy\Cinegy Air PRO" -PassThru
...
## Auto Launch Playout Dashboard using Task Scheduler
$file = Get-S3Object -BucketName $bucket -Key 'dashboard-launch.ps1'
Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\UserData'
# Define the action to run a PowerShell script
$action = New-ScheduledTaskAction -Execute 'PowerShell.exe' -Argument '-ExecutionPolicy Bypass -File "C:\UserData\dashboard-launch.ps1"'
...
Register-ScheduledTask -Action $action -Trigger $trigger -TaskName "CinegyTask" -Description "Task to execute powershell script related to cinegy" -User "NT AUTHORITY\SYSTEM" -Settings $settings -RunLevel Highest -TaskPath "\"

####################Automate the deleting older file using Task Scheduler################################################
#$file = Get-S3Object -BucketName $bucket -Key 'dltoldfiles.ps1'
#Copy-S3Object -SourceBucket $bucket -SourceKey $($file.Key) -LocalFolder 'C:\UserData'
# Define the script or command to be scheduled
#$scriptPath = "C:\UserData\dltoldfiles.ps1"
# Define the action to run a PowerShell script
...
Start-Sleep -Seconds 300 #Add a 5 mins seconds delay
write-host("computer Joined to Corp Domain retarting the system") 
